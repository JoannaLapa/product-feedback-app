<template>
  <header>
    <div
      class="top-0 flex flex-col sm:mt-14 sm:mb-10 sm:grid sm:max-h-[11.125rem] sm:grid-cols-3 sm:grid-rows-1 sm:gap-2.5 lg:my-0 lg:max-h-[33.0625rem] lg:grid-cols-1 lg:grid-rows-[8.5625rem_23rem] lg:gap-6 xl:w-63"
    >
      <div
      ...
      <div
        class="sm: absolute top-[72px] right-0 z-20 flex h-screen w-3/4 self-end bg-neutral-200 before:absolute before:-left-2/4 before:h-screen before:w-[50%] before:bg-black before:opacity-50 sm:static sm:col-start-2 sm:col-end-4 sm:flex sm:max-h-full sm:w-full sm:before:hidden lg:left-0 lg:col-span-1"
        :class="{ hidden: isOpened }"
      >
        <ul
          class="flex flex-col gap-5 p-6 sm:grid sm:grid-cols-2 sm:grid-rows-1 sm:gap-2.5 sm:p-0 lg:grid-cols-1 lg:grid-rows-[10.375rem_11.125rem] lg:gap-6"
        >
          <SortByCategory />
          <RoadmapMenu />
        </ul>
