<template>
  <header>
    <div
      class="max-sm:fixed max-sm:w-screen top-0 flex flex-col sm:mt-14 sm:mb-10 sm:grid sm:max-h-[11.125rem] sm:grid-cols-3 sm:grid-rows-1 sm:gap-2.5 lg:my-0 lg:max-h-[33.0625rem] lg:grid-cols-1 lg:grid-rows-[8.5625rem_23rem] lg:gap-6 xl:w-63"
    >
      <div
      ...
      <div
        class="max-sm:w-3/4 max-sm:before:absolute max-sm:before:-left-2/4 max-sm:before:h-screen max-sm:before:w-[50%] max-sm:before:bg-black max-sm:before:opacity-50 relative right-0 flex self-end bg-neutral-200 sm:col-start-2 sm:col-end-4 sm:flex sm:max-h-full lg:left-0 lg:col-span-1"
        :class="{ hidden: isOpened }"
      >
        <ul
          class="max-sm:h-screen max-sm:p-6 flex flex-col gap-5 sm:grid sm:grid-cols-2 sm:grid-rows-1 sm:gap-2.5 lg:grid-cols-1 lg:grid-rows-[10.375rem_11.125rem] lg:gap-6"
        >
          <SortByCategory />
          <RoadmapMenu />
        </ul>
